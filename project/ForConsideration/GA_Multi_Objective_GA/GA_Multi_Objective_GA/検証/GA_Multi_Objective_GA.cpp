...
int main(void)
{
	...
	/***** 最終世代の各目的関数の値 *****/
	// 最終世代の適応度をfinal_generation_fitness.txtに書き込み
	FILE *f_final_fitness;
	fopen_s(&f_final_fitness, "final_generation_fitness.txt", "w");
	for (i = 0; i < IND_NUM; i++) {
		fprintf(f_final_fitness, "%d\n", fitness[i]);
	}
	fclose(f_final_fitness);
