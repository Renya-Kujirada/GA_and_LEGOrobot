// GA_Last_generation_Pareto_20180112_考察用実験プログラム(最終世代のパレート解のみを求めるためのプログラム)
...
#define Restriction 1800				//目的関数の制約条件（速度合計値の下限値）
#define Penalty 10000					// ペナルティ
...
int main(void)
{
	...
	for (g = 0; g < generation; g++) {	// generation回進化する
		...
		// 適合度の計算
		for (i = 0; i < IND_NUM; i++) {
			fitness[i] = 4*speed_sum[i] - speed_change[i];
	...
	/***** パレート解の探索 *****/
	int Pareto_NUM = 0;
	int Pareto_speed_sum[IND_NUM] = {};
	int Pareto_speed_change[IND_NUM] = {};
	int inferiority_flag = 0;
